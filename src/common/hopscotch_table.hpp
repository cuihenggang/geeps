...
class hopscotch_table {
 ...
 public:
    ...
    struct insert_iterator {
      ...
      explicit insert_iterator(hopscotch_table* c):cmap(c) {}
 ...
 private:
    ...
    iterator insert_impl(const value_type& newdata, bool overwrite = true) {
      ...
      // insert and return
      // we need to lock ID - 1 so as to ensure intersection with the
      // hash target
    ...
    iterator find_impl(const value_type& key, size_t target) {
      const_iterator iter =
        ((const hopscotch_table*)(this))->find_impl(key, target);
